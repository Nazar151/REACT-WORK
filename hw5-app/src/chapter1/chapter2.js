   ...
   const response = await fetch(`${baseUrl}/${endpoint.trim().toLowerCase()}/${id.trim()}`)
    ...
    return (
        <div>
            ...
            <br />
            <br />
            <input value={endpoint} onChange={({target:{value}}) => setEndPoint (value)} type='type' placeholder='E.g posts, comments,todos etc'/>
            <br />
            <br />
            <input value={id} onChange={({target:{value}}) => setId (value)} type='type' placeholder='resource id, e.g 1,2,3'/>
            <br />
            <br />
